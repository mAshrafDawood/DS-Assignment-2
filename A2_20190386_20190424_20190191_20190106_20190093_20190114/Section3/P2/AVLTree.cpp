...
AVLTree* AVLTree::insertValue(AVLTree* node, int newValue) {
    ...
    if (balance > 1 && newValue < node->left->value) { // Left Left
		return rightRotation(node);
	}
	else if (balance < -1 && newValue > node->right->value) { // Right Right
		return leftRotation(node);
	}
	else if (balance > 1 && newValue > node->left->value) { // Left Right
		node->left = leftRotation(node->left);
		return rightRotation(node);
	}
	else if (balance < -1 && newValue < node->right->value) { // Right Left
